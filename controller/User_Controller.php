    ...
    function register($params){
		...
		if (empty($nama_depan) || empty($nama_belakang) || empty($username) || empty($email) || empty($password) || empty($konfirmasi_password)) {
			...
			if ($username != str_replace(' ', '', $username)) {
				$pesan_form = '<p class="text-danger">Username tidak boleh menggunakan spasi!</p>';
				goto output;
			}
			...
			if(!$tambah){
				...
				$pesan_form = '<p class="text-success">Anda berhasil mendaftar, silahkan lakukan login kembali!</p>';
			}
